...
makeInsertAndUpdateChampionObj = ->
    for match in Matches.find({region: 'BR'}).fetch()
        ...
        for participant in match.participants
            ...
            if Champions.find({id: championId}).count() == 0
                url = 'https://global.api.pvp.net/api/lol/static-data/'+championRegion.toLowerCase()+'/v1.2/champion/'+championId+'?champData=image&api_key='+apiKey
